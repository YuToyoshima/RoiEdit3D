...
public class MyTiffEncoder{
    ...
    // followings are came from ij.io.TiffDecoder.
    //constants
    ...
    public MyTiffEncoder (FileInfo fi) {        
        this.fi = fi;
    ...
    public void write_singleIFD(OutputStream out) throws IOException {
        writeHeader(out);
        long nextIFD = 0L;
    ...
    void writeEntry(OutputStream out, int tag, int fieldType, int count, int value) throws IOException {
        ...
        if (count==1 && fieldType==this.SHORT) {
    ...
    void writeIFD(OutputStream out, int imageOffset, int nextIFD) throws IOException {  
        ...
        if (fi.fileType==fi.GRAY32_FLOAT) {
            ...
            int format = this.FLOATING_POINT;
