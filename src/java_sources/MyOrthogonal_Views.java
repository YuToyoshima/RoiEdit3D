...
public class MyOrthogonal_Views implements ij.plugin.PlugIn, MouseListener, MouseMotionListener, KeyListener, ActionListener,
ImageListener, WindowListener, AdjustmentListener, MouseWheelListener, FocusListener, CommandListener {
    ...
    private static MyOrthogonal_Views instance;
...
// added from ij.CompositeImage
    ...
    LUT[] luts;
    ImageProcessor[] cip;
    static Overlay overlay_xy, overlay_xz, overlay_yz;
    ...
    public void setOverlays(Overlay overlay_xy, Overlay overlay_xz, Overlay overlay_yz) {
        this.overlay_xy = overlay_xy;
        this.overlay_xz = overlay_xz;
        this.overlay_yz = overlay_yz;
        imp.getCanvas().setShowAllList(overlay_xy);
        xz_image.getCanvas().setShowAllList(overlay_xz);
        yz_image.getCanvas().setShowAllList(overlay_yz);
    }
    
    public Overlay[] getOverlays() {
        Overlay[] ret = new Overlay[3];
        ret[0] = overlay_xy;
        ret[1] = overlay_xz;
        ret[2] = overlay_yz;
        return ret;
    ...
    public Overlay[] getCanvasOverlays() {
        Overlay[] ret = new Overlay[3];
        ret[0] = imp.getCanvas().getShowAllList();
        ret[1] = xz_image.getCanvas().getShowAllList();
        ret[2] = yz_image.getCanvas().getShowAllList();
        return ret;
    }
...
//	    }
            ...
            if (imp.isComposite()) {
                ...
                active[0] = true;
                luts = new LUT[7];
                luts[0] = imp.getProcessor().getLut();
            }
            cip = new ImageProcessor[nChannels];
            rgbPixels= new int[imp.getHeight()*imp.getWidth()];
            //rgbImage = new BufferedImage(imp.getWidth(),imp.getHeight(),BufferedImage.TYPE_INT_RGB);
            
            for (int j=0; j<slices; j++) {
                for (int i=0; i<nChannels; ++i) {
                    cip[i] = stack2.getProcessor(imp.getStackIndex(i+1,j+1,t));
                    cip[i].setLut(luts[i]);
                }
                rgbImage = new BufferedImage(imp.getWidth(),imp.getHeight(),BufferedImage.TYPE_INT_RGB);
                updateImage();
                stack.addSlice(null, new ColorProcessor(rgbImage));
            }
...
// copyied and modified from ij.CompositeImage
    synchronized void updateImage() {
        
        int imageSize = imp.getWidth()*imp.getHeight();
        int nChannels = imp.getNChannels();
...
// copied and modified from ij.CompositeImage
    ...
    void createBufferedImage() {
        ...
        DataBuffer dataBuffer = new DataBufferInt(rgbPixels, imp.getWidth()*imp.getHeight(), 0);
...
// copied and modified from ij.CompositImage
    SampleModel getRGBSampleModel() {
        ...
        sampleModel = sampleModel.createCompatibleSampleModel(imp.getWidth(), imp.getHeight());
        return sampleModel;
    }
    ...
    // copied and modified from ij.gui.Overlay
    void updateViews(Point p, ImageStack is) {
        ...
        ic = yz_image.getCanvas();
        ml = ic.getMouseListeners();
        flag_ml = false;
        for (int i=0; i<ml.length; i++) {
            if (ml[i].equals(this)) { flag_ml = true; }
        }
        if (!flag_ml) {
...
////////////////////////////////////////////////////////////////////////
    ...
    public static MyOrthogonal_Views getInstance() {
        return instance;
    }
...
//                     IJ.error("MyOrthogonal_views","An error occured.\n \n" + msg);
                    e.printStackTrace();
                }
            }
        }
